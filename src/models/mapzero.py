...
class MapZero(AbstractNetwork):
    def __init__(self,dim_feat_dfg,dim_feat_cgra,out_dim,n_heads,len_action_space,dtype,enviroment:InterfaceEnviroment):
        super().__init__()
        negative_slope = 0.02
    ...
    def recurrent_inference(self,states,actions): 
        ...
        for state,action in zip(states,actions):
            next_state,reward,_ = self.enviroment.step(state,action.item())
...
#     model = MapZero(4,torch.float)
#     dfg_data = torch.rand(2,10,dtype=torch.float)
#     cgra_data = torch.rand(4,7,dtype=torch.float)
#     nodes_data = torch.rand((10),dtype=torch.float)
#     edges_index_dfg = torch.tensor([[0],[1]],dtype = torch.long)
#     edges_index_cgra = torch.tensor([[0,1,2],[1,2,3]],dtype=torch.long)
#     assert model(dfg_data,edges_index_dfg,cgra_data,edges_index_cgra,nodes_data).shape[0] == 32
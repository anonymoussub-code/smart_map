...
class YOTTDFG(MappingGraphInterface):
    def __init__(self, graph:GraphInterface,node_to_operation:dict): 
        self.special_value = -1

        self.zigzag_edges,_,cycles = Zigzag.get_zigzag_order(graph.get_edges())
        _ = None
        placement_order = [*self.zigzag_edges[0]] + [v for (_,v) in self.zigzag_edges[1:]]
        self.annotations = list(Zigzag.get_graph_annotations(self.zigzag_edges,cycles).values())
        self.base_dfg : DFG = DFG(graph,node_to_operation,placement_order,self.special_value)
        self.alap_values  = ALAP.get_alap_values(self.base_dfg.vertices,self.base_dfg.edges)
    ...
    def get_feature_by_node_id(self, node_id):    
        return {
            ...
            'num_vertex_same_modulo': self.number_vertices_to_be_mapped_same_modulo_time_slice_by_node_id(node_id),

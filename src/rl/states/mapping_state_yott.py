...
import torch
from src.graphs.dfgs.yott_dfg import YOTTDFG
from src.graphs.cgras.cgra_traversal import CGRATraversal
import numpy as np
class MappingStateYOTT(MappingStateInterface):
    def __init__(self,dfg: YOTTDFG, cgra: CGRATraversal, id_node_to_be_mapped,distance_func):
        self.dfg = dfg
        self.cgra = cgra
    ...
    #     self.bad_reward = len(self.dfg.base_dfg.edges)*(cgra.cgra.dim_arch[0] + cgra.cgra.dim_arch[1] - 2)
        self.bad_reward = 100

    def is_bad_reward(self,reward):
        #From enviroment -reward/100
        return -reward*100 >= self.bad_reward

    # def is_bad_reward(self,reward):
    #     return reward**2 >= self.bad_reward
    ...
    def __get_legal_actions(self):
        ...
        if self.id_node_to_be_mapped is not None:
            ...
            annotations = self.dfg.annotations[edge_index]
            if annotations == []:
                return [v for v in self.cgra.out_vertex[pe_mapped_node] if  self.cgra.cgra.pe_to_dfg_node[v] == self.cgra.fill_value] 
            macthed_pes = []
            
            for map_node_annot,dist in annotations:
                map_node_annot = self.dfg.base_dfg.real_to_reseted_node[map_node_annot]
                pe_mapped_node_annot = self.dfg.base_dfg.node_to_pe[map_node_annot]
                pe_mapped_node_annot_pos = self.cgra.get_pe_pos_by_pe_id(pe_mapped_node_annot)
                cur_matched_pes = set()
                for vertice in self.cgra.cgra.graph.get_vertices():
                    vertice_pos = self.cgra.get_pe_pos_by_pe_id(vertice)
                    cur_dist = self.distance_func(vertice_pos,pe_mapped_node_annot_pos)
                    if cur_dist <= dist + 1 and self.cgra.cgra.pe_to_dfg_node[vertice] == self.cgra.fill_value:
                        cur_matched_pes.add(vertice)
                macthed_pes.append(cur_matched_pes)
            
            actions = []
            for neigh in self.cgra.out_vertex[pe_mapped_node]:
                match = True
                for pes in macthed_pes:
                    if neigh not in pes:
                        match = False
                        break
                if match:
                    actions.append(neigh)
            return actions
        return []

...
class UtilMapping:
    ...
    def mapping_is_valid_2(vertices,edges_or_max_root,node_to_pe,node_to_in_vertices,node_to_out_vertices,pes_to_routing):
        ...
        def schedule(node,in_vertices,cur_schedule,node_to_scheduled_time_slice,fifo):
            ...
            # father_pe = node_to_pe[node]
            # for child in node_to_out_vertices[node]:
            #     temp_child_pe = node_to_pe[child]
            #     node_to_scheduled_time_slice[child] = node_to_scheduled_time_slice[node] +  len(pes_to_routing[(father_pe,temp_child_pe)])-1
            #     visited[child] = True
            #     fifo.append(child)
            #     for father in node_to_in_vertices[child]:
            #         if father not in node_to_scheduled_time_slice:
            #             temp_father_pe = node_to_pe[father]
            #             schedule(father,in_vertices,node_to_scheduled_time_slice[child] -(len(pes_to_routing[(temp_father_pe,temp_child_pe)])-1),node_to_scheduled_time_slice,fifo)

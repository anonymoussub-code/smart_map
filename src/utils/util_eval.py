...
class UtilEval:
    @staticmethod
    def calculate_mean_expanded_nodes(model,config,observation,enviroment,num_max_expansion,max_moves):
        """
        ...
        with torch.no_grad():
            ...
            # Choose the action
                ...
                while count < len(root.children):
                    ...
                    observation, reward, done = enviroment.step(root.state,action)
    ...
    @staticmethod
    def play_game(config,model,initial_state,enviroment,num_max_expansion = None):
        """
        Play one game with actions based on the Monte Carlo tree search at each moves.
        """

...
class UtilInterconnections:
    ...
    @staticmethod
    def generate_crossbar_interconnection_by_pe_pos(pe_pos,dim_cgra):
        """
        Generates a crossbar connections for a PE at a given position (pe_pos).
        ...
        Returns:
            list: Crossbar interconnections of the PE at the position 'pe_pos'.

        Tested:
            True
        """
        adj_positions = [(i,j) for i in range(-dim_cgra[0] + 1 ,dim_cgra[0]) for j in range(-dim_cgra[1]+1,dim_cgra[1]) if (i,j) != (0,0)]         
        return UtilInterconnections.generate_interconnection_by_adj_positions_and_pe_pos(adj_positions,pe_pos,dim_cgra)

...
def infer(model_config):
    ...
    for state,dfg_name in states:
        ...
        model_launcher = ModelLauncher(model,dfg_name,config,state,enviroment,checkpoint,None)
        ...
        if mapping is None:
            ...
            mapping, max_tree_depths, mcst_simu_flag, num_backtrackings = UtilEval.play_game(config,model_cp,state,enviroment,config.num_max_expansion_test)
            ...
            mapping_wo_finetune, max_tree_depths, mcst_simu_flag, num_backtrackings = UtilEval.play_game(config,model_cp,state,enviroment,config.num_max_expansion_test)
        ...
        mean_visited_rate,mean_visited_nodes,mean_mean_expanded_nodes = UtilEval.calculate_mean_expanded_nodes(model_cp,config,state,enviroment,config.num_max_expansion_test,config.max_moves) 

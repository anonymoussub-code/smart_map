...
def infer(model_config):
    ...
    for state,dfg_name in states:
        ...
        mapping, max_tree_depths, mcst_simu_flag, num_backtrackings = UtilEval.play_game(config,model,state,enviroment,config.num_max_expansion_test)
        ...
        # print('DFG node | Scheduled Time')
        # if final_state.node_to_scheduled_time_slice:
        #     node_to_time_slice = final_state.node_to_scheduled_time_slice
        #     map_nodes = final_state.dfg.base_dfg.reseted_to_real_node
        #     max_key_length = max(len(map_nodes[key]) for key in node_to_time_slice)
        #     max_value_length = max(len(str(value)) for value in node_to_time_slice.values())
    
        #     for key, value in node_to_time_slice.items():
        #         node_name = map_nodes.get(key, 'Unknown')
        #         print(f"\t{str(node_name).ljust(max_key_length)} | {str(value).ljust(max_value_length)}")

        # else:
        #     assert not mapping_is_valid
        #     print('None, invalid placement or routing.')

        # print()
        mean_max_tree_depths = numpy.mean(max_tree_depths)
        mean_visited_rate,mean_visited_nodes,mean_mean_expanded_nodes = UtilEval.calculate_mean_expanded_nodes(model,config,state,enviroment,config.num_max_expansion_test,config.max_moves) 
        print(f'MCTS results: Mean visited rate: {mean_visited_rate*100:.3f}% | Mean visited nodes: {mean_visited_nodes:.3f} | 2xMean expanded nodes: {mean_mean_expanded_nodes:.3f} | Mean max tree depth {mean_max_tree_depths:.3f} | Number of Backtrackings: {num_backtrackings}')
        
        mapping_results = MappingResults(config.model_name.value,arch_dims,model_config.type_interconnnections.value,dfg_name.replace('.dot',''),EnumMode.ZERO_SHOT.value)
...
if __name__ == "__main__":
    ...
    # config_class = 'test'
    # model_config = ConfigMapzero(EnumInterconnections.OH_TOR_DIAG,(4,4),EnumMode.TEST)
    infer(model_config)
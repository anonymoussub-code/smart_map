...
@torch.no_grad
def generate_mappings(initial_states,enviroment,model,old_model,is_mapzero,eval = False):
    model.eval()
    mappings = []
    states = []
    actions = []
    ...
    for i,initial_state in enumerate(initial_states):
        ...
        while not is_end_state:
            ...
            if not eval:
                ...
                old_action_probs.append(old_p.squeeze()[action_indice].item())
                old_values.append(old_v.item())
            cur_state, reward,is_end_state = enviroment.step(cur_state,action)
    ...
    return [states,actions,action_indices,rewards,values,policy_probs,all_returns,old_action_probs,old_values]
...
def loss_function(target_values,values,old_values,probs,old_action_probs,action_indices,w_entropy_loss=0.05  ,w_v_loss=1,w_kl = 1,clip_factor = 0.2):

    advantages = (target_values - values).detach()
    if target_values.size(0) > 1:
        advantages = (advantages - advantages.mean())/(advantages.std() + 1e-8)

    dist = Categorical(probs)
    cur_log_prob = dist.log_prob(action_indices)
    log_old_action_probs = old_action_probs.log()
    ratio = torch.exp(cur_log_prob - log_old_action_probs)

    kl_loss = torch.mean(cur_log_prob - log_old_action_probs)
    ...
    value_loss_1 = torch.nn.functional.mse_loss(target_values, values)
    value_loss_2 = torch.nn.functional.mse_loss(target_values, value_clip)
    ...
    return total_loss,w_v_loss*value_loss.item(),policy_loss.item(), w_entropy_loss*entropy_loss.item(), w_kl*kl_loss.item()
...
def train(model_config,batch_size):
    ...
    cgra = cgra = model_config.get_cgra()
    ...
    for file in files:
        # if file == 'benchmarks/synthetics/4/V_4_E_4.dot':
    ...
    total_train_time = 0
    # min_loss = float('inf')
    # max_buffer = (1) * len(initial_states)
    # buffer = [[] for _ in range(9)]
    # best_mappings = {}
    # max_buffer = 4 * len(initial_states) * 16
    ...
    for epoch in range(config.epochs):
        temp_mappings = generate_mappings(initial_states,enviroment,model,old_model,is_mapzero,eval=False)
        # for i,array in enumerate(temp_mappings):
        #     buffer[i] += array
        # if len(buffer[0])> max_buffer:
        #     for i,array in enumerate(temp_mappings):
        #         buffer[i] = buffer[i][-max_buffer:]
        # print(len(buffer[0]))
        old_model = copy.deepcopy(model)
        ...
        for (states,actions,action_indices,rewards,batch_values,policy_probs,returns,old_action_probs,old_values) in train_dataloader:
            optimizer.zero_grad()
            ...
            total_loss,value_loss,policy_loss, entropy_loss,kl_loss = loss_function(returns,values.squeeze(-1),old_values,policy_probs.squeeze(0),old_action_probs,action_indices,w_entropy_loss,1,c_kl,0.2)
            total_loss.backward()   
        ...
        temp_rewards = generate_mappings(initial_states,enviroment,model,None,is_mapzero,eval=True)[3]
        ...
        epoch_entropy_loss/=len(train_dataloader)
        ...
        print(f'Epoch: {epoch + 1}. Total Loss: {epoch_total_loss}. Value Loss: {epoch_value_loss:.5f}. Policy Loss: {epoch_policy_loss:.5f}. Entropy Loss: {epoch_entropy_loss:.3f}. KL Loss: {epoch_kl_loss:.3f} | Eval: Mean Mean Reward: {epoch_mean_rewards:.4f} | lr: {lr:.6f}. Time: {final_train_time-init_train_time:.3f}')

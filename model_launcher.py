...
import warnings
class ModelLauncher:
    def __init__(self,model,dfg_name,config: Config, initial_state, enviroment, checkpoint, path_replay_buffer, split_resources_in=1):
        ...
        # cpu_weights = cpu_actor.get_initial_weights.remote(self.model)
        # print(self.checkpoint['weights']['policy_net.fc.bias'])
    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                # writer.add_scalar(
                #  "Self_played_games/Step",
                #     info["num_played_games"],
                #     counter,
                # )
              
                # writer.add_scalar(
                #     "Self_played_steps/Step", info["num_played_steps"], counter
                # )

                # writer.add_scalar(
                #     "Reanalysed_games/Step",
                #     info["num_reanalysed_games"],
                #     counter,
                # )

                # writer.add_scalar(
                #     "Training_steps_per_self_played_step_ratio/Step",
    ...
    def test(
        ...
        self_play_worker = SelfPlay.options(
            num_cpus=0,
            num_gpus=num_gpus,
        ).remote(self.checkpoint, self.model, self.enviroment,self.config,initial_state, self.config.seed)
        results = []
    ...
    def diagnose_model(self, horizon):
        ...
        dm.get_real_trajectories(self.initial_state, horizon,self.enviroment)
